...
def process_law_file(file_path):
    ...
    for row in table.find_all('tr'):
        ...
        with open(os.path.join(save_path, f"{title}.txt"), 'w', encoding='utf-8') as f:
            f.write(f"【법령】\n{clean_korean_law_text(cols[0].get_text(separator='\\n'))}\n\n")
            f.write(f"【시행령】\n{clean_korean_law_text(cols[1].get_text(separator='\\n'))}\n\n")
            f.write(f"【시행규칙】\n{clean_korean_law_text(cols[2].get_text(separator='\\n'))}")
