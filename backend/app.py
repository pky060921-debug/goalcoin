...
app = Flask(__name__)
# 🚨 500 에러 시에도 무조건 통과하도록 CORS 강제 적용 및 50MB 허용
...
def handle_exception(e):
    ...
    print(f"\n[🚨 백엔드 에러 발생]\n{error_detail}")
    response = jsonify({"error": "백엔드 에러", "details": error_detail})
...
def upload_pdf():
    ...
    try:
        ...
        if filename.endswith('.pdf'):
            ...
            if docx is None:
                return jsonify({"error": "docx 모듈이 없습니다. 터미널에서 pip3 install python-docx 를 실행하세요."}), 500
            import io
            doc_file = docx.Document(io.BytesIO(raw_bytes))
            full_text = "\n".join([p.text for p in doc_file.paragraphs])
        elif filename.endswith(('.html', '.htm')):
            html_data = raw_bytes.decode('utf-8', errors='ignore')
            full_text = re.sub(r'<[^>]+>', ' ', html_data)
        else:
            # HWP 및 알 수 없는 포맷 강제 텍스트 추출
            try:
                full_text = raw_bytes.decode('utf-8')
            except:
                try:
                    full_text = raw_bytes.decode('cp949')
                except:
                    decoded_str = raw_bytes.decode('utf-8', errors='ignore')
                    clean_text = re.sub(r'[^\uAC00-\uD7A3a-zA-Z0-9\s.,()\[\]]', ' ', decoded_str)
                    full_text = re.sub(r'\s+', ' ', clean_text)

        if not full_text.strip():
            return jsonify({"error": "문서에서 글자를 추출할 수 없습니다."}), 400
