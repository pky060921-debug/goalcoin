...
# 💡 [핵심 수정] Gemini 자동생성 라우터 (내용 중심 스캔 프롬프트 적용)
...
# 💡 [핵심 수정] 로컬 AI 라우터 (내용 중심 풀스캔 프롬프트 적용)
...
def analyze_chunk():
    try:
        ...
        [검색 꿀팁 💡] 조항 이름(제1조 등)이나 파일명에 너무 집착하지 마세요! 이름이 약간 달라도 [참고 자료 DB]의 **'실제 본문 내용'**을 전체적으로 훑어보고 지문과 의미가 일치하는 문장이 있다면 유연하게 정답의 근거로 인정하세요.
