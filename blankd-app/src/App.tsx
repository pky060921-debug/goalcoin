...
// 🚨 오류 진단: 리액트 컴포넌트 렌더링 중 발생하는 에러를 캐치하는 에러 바운더리
class ErrorBoundary extends React.Component<any, any> {
  ...
  componentDidCatch(error: any, errorInfo: any) {
    console.error("오류 진단 - 프론트엔드 렌더링 에러:", error, errorInfo);
...
// --- 🚨 [신규 추가] 법-령-칙 3단 비교 레이아웃 컴포넌트 ---
const LawLayout = () => {
  const [data, setData] = useState({ law: [] as any[], decree: [] as any[], rule: [] as any[] });
  const [errorLog, setErrorLog] = useState("");

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://api.blankd.top/api/law_data');
        if (!response.ok) throw new Error(`HTTP 통신 오류 진단: 상태 코드 ${response.status}`);
        
        const result = await response.json();
        if (result.status === 'success') {
          setData(result.data);
        } else {
          throw new Error(`백엔드 내부 오류 진단: ${result.message}`);
        }
      } catch (error: any) {
        console.error("오류 진단 - 데이터 로드 실패:", error);
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      <h2 className="text-xl font-light tracking-widest text-indigo-400 mb-6 uppercase">⚖️ 법-령-칙 3단 비교 시스템</h2>
      
      {errorLog && (
        <div className="bg-rose-950/20 border-l-4 border-rose-500 p-4 mb-6">
          <strong className="text-rose-400 text-sm">오류 진단:</strong> <span className="text-white/70 text-xs">{errorLog}</span>
        </div>
      )}

      <div className="flex flex-col md:flex-row gap-6">
        {/* 1. 법 (Law) */}
        <div className="flex-1 border border-white/10 p-6 rounded-sm bg-white/[0.02]">
          <h3 className="text-sm border-b border-indigo-500/50 pb-3 mb-4 text-indigo-300">법 (Law)</h3>
          {data.law.map(item => (
            <div key={item.id} className="mb-4 bg-black/20 p-4 border border-white/5 rounded-sm">
              <strong className="block text-xs text-indigo-400 mb-2">{item.article}</strong>
        ...
        {/* 2. 시행령 (Decree) */}
        <div className="flex-1 border border-white/10 p-6 rounded-sm bg-white/[0.02]">
          <h3 className="text-sm border-b border-teal-500/50 pb-3 mb-4 text-teal-300">시행령 (Decree)</h3>
          {data.decree.map(item => (
            <div key={item.id} className="mb-4 bg-black/20 p-4 border border-white/5 rounded-sm">
              <strong className="block text-xs text-teal-400 mb-2">{item.article}</strong>
              <p className="text-xs text-white/80 leading-relaxed">{item.content}</p>
            </div>
          ))}
        </div>
        ...
        {/* 3. 시행규칙 (Rule) */}
        <div className="flex-1 border border-white/10 p-6 rounded-sm bg-white/[0.02]">
          <h3 className="text-sm border-b border-amber-500/50 pb-3 mb-4 text-amber-300">시행규칙 (Rule)</h3>
          {data.rule.map(item => (
            <div key={item.id} className="mb-4 bg-black/20 p-4 border border-white/5 rounded-sm">
              <strong className="block text-xs text-amber-400 mb-2">{item.article}</strong>
              <p className="text-xs text-white/80 leading-relaxed">{item.content}</p>
            </div>
          ))}
        </div>
...
function MainApp() {
  ...
  const [panelState, setPanelState] = useState({
    status: 'idle', title: '시스템 대기 중', message: '법령 문헌을 선택하거나 분석 개시 버튼을 눌러주세요.',
    current: 0, total: 0, logs: [] as string[]
  });
  ...
  useEffect(() => {
    const handleAuth = async () => {
      ...
      } catch (err: any) { console.error("오류 진단:", err); }
    };
  ...
  const loadCategories = async () => {
    ...
    } catch (err) { console.error("오류 진단:", err); }
  };
  ...
  const uploadFile = async (type: 'law' | 'exam') => {
    ...
    try {
      ...
      updatePanel('error', '네트워크 연결 끊김 (Failed to Fetch)', `오류 진단 - 서버 통신 불가: ${error.message}`);
      alert(`[🚨 치명적 연결 오류]\n서버와 통신할 수 없습니다.`);
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`https://api.blankd.top/api/${endpoint}`, { method: "POST", body: formData });
  ...
  const handleAutoMakeCard = async (cat: Category, silent = false) => {
    ...
    try {
      ...
      updatePanel('error', '오류 진단 - 추출 실패', err.message);
    }
  };
  ...
  const handleDeleteCategory = async (cat_id: number) => {
    ...
    try {
      ...
      if (res.ok) { alert("삭제되었습니다."); loadCategories(); }
    } catch (err) { alert("오류 진단 - 삭제 실패"); }
  };
  ...
  const handleDeleteCard = async (card_id: number) => {
    ...
    try {
      ...
      if (res.ok) { alert("카드가 삭제되었습니다."); loadMyCards(); }
    } catch (err) { alert("오류 진단 - 삭제 실패"); }
  };
  ...
  const handleGoogleZkLogin = async () => {
    try {
      ...
      const url = await createUrl.call(enokiFlow, {
        provider: 'google', clientId: '536814695888-bepe0chce3nq31vuu3th60c7al7vpsv7.apps.googleusercontent.com',
        redirectUrl: window.location.origin, network: 'testnet'
      });
      window.location.href = url;
    } catch (err: any) { alert(`오류 진단 - 로그인 에러: ${err.message}`); }
  };
  ...
  const handleGithubPull = async () => {
    ...
    } catch (err) { alert("오류 진단 - 서버 연결 실패"); }
  };

  const loadTextForManualSelection = async (cat: Category) => {
    setParsedText(cat.content); setSelectedWordIndices(new Set()); setAiExplanation("");
    ...
    } catch (err) { updatePanel('error', '오류 진단 - 검색 실패', '관련 기출문제 로드 중 에러가 발생했습니다.'); }
  };
  ...
  const getAiExplanation = async (exam: any) => {
    ...
    } catch (err) { setAiExplanation("오류 진단 - 서버와의 통신 오류로 해설을 가져오지 못했습니다."); }
  };
  ...
  const handleMakeBlankCard = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSelectedWordIndices(new Set()); setParsedText(""); loadMyCards();
        updatePanel('success', '저장 완료', '카드가 성공적으로 추가되었습니다.');
      }
    } catch(err:any) { updatePanel('error', '오류 진단 - 저장 실패', err.message); }
  };
          ...
          <>
            <nav className="flex gap-8 mb-16 border-b border-white/5 pb-4 overflow-x-auto scrollbar-hide">
              {[
                ...
                { id: 'law_viewer', label: '3단 비교' }, // 🚨 신규 탭 추가
                { id: 'mypage', label: '설정' }
              ].map((tab) => (
                <button
                  key={tab.id}
            ...
            {/* 🚨 신규 법-령-칙 레이아웃 연동 */}
            {activeTab === 'law_viewer' && <LawLayout />}
                        ...
                        {categories.map(cat => (
                          <div key={cat.id} className="border border-white/5 p-4 flex justify-between items-center group bg-white/[0.01]">
                            ...
                            <div className="flex gap-2">
                              <button onClick={() => handleDeleteCategory(cat.id)} className="text-[10px] border border-rose-900/30 text-rose-500/60 px-3 py-1.5 hover:border-rose-500/50 hover:bg-rose-900/20 whitespace-nowrap">삭제</button>
                              <button onClick={() => handleAutoMakeCard(cat)} className="text-[10px] border border-white/10 px-3 py-1.5 hover:border-white/40 whitespace-nowrap">분석</button>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div ref={textRef} className="font-serif text-[14px] leading-[2.5] text-white/70 h-48 overflow-y-auto border border-white/10 p-5 bg-[#0a0a0c] scrollbar-hide break-all">
                        {parsedText.split(SPLIT_REGEX).map((word, idx) => {
                          ...
                          return (
                            <span 
                              key={idx} onClick={() => toggleWordSelection(idx)} 
                      ...
                      <div className="space-y-6 overflow-y-auto max-h-[500px] scrollbar-hide pr-2">
                        {relatedExams.map((exam, i) => (
                          <div key={i} className="p-4 bg-white/5 border border-white/10 rounded-sm">
                            ...
                            {exam.explanation && exam.explanation.trim() !== "" && (
                              <p className="text-[10px] text-white/40 mb-4 bg-black/50 p-2 border border-white/5">기존 해설: {exam.explanation}</p>
                            )}
                            <button onClick={() => getAiExplanation(exam)} className="text-[9px] px-3 py-1.5 border border-indigo-500/50 text-indigo-300 hover:bg-indigo-500/10 tracking-widest uppercase">
                        ...
                        {aiExplanation && (
                          <div className="mt-6 p-5 border border-indigo-500/30 bg-indigo-950/20 rounded-sm">
                            ...
                            <div className="text-[11px] leading-relaxed text-white/80 font-serif">{aiExplanation}</div>
                          </div>
                        )}
                      </div>
                    ) : (
                      <div className="h-40 flex items-center justify-center text-[10px] text-white/20 uppercase tracking-widest">관련된 기출문제가 없습니다.</div>
                    )}
                  </div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {savedCards.map((card) => (
                      <div key={card.id} onClick={() => card.status !== "BURNED" && setActiveCard(card)}
                        className={`border p-6 transition-all cursor-pointer relative bg-white/[0.01] rounded-sm ${card.status === "BURNED" ? "border-white/5 opacity-30" : getTierClass(card.level)}`}
                      >
                        <div className="flex justify-between items-start mb-6">
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            {card.status !== "BURNED" && (
                              <button onClick={(e) => { e.stopPropagation(); handleDeleteCard(card.id); }} className="text-[10px] text-rose-500/60 hover:text-rose-400 tracking-widest">삭제</button>
                            )}
                          </div>
                        </div>
