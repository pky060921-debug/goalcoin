...
function App() {
  ...
  const handleMakeBlankCard = () => {
    ...
    if (!selection || selection.toString().trim() === "") {
      return alert("빈칸으로 뚫을 조문 키워드를 드래그해주세요!");
    }
      ...
      <main className="max-w-md mx-auto space-y-6">
        <section className="bg-slate-900 border border-slate-800 p-6 rounded-3xl space-y-4 shadow-xl">
          ...
          <input 
            ...
            className="block w-full text-sm text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-500 transition-all"
          />
          <button 
            onClick={handleFileUpload} disabled={isUploading || !file}
            className="w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-3 rounded-xl flex justify-center items-center gap-2"
          >
        ...
        {parsedText && (
          <section className="bg-slate-900 border border-slate-800 p-6 rounded-3xl space-y-4 shadow-xl">
            ...
            <div ref={textRef} className="bg-slate-950 p-4 rounded-xl text-sm leading-relaxed text-slate-300 h-48 overflow-y-auto border border-slate-800">
              {parsedText}
            </div>
            <button 
              onClick={handleMakeBlankCard}
              className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl flex justify-center items-center gap-2"
            >
          ...
          <section className="bg-slate-900 border border-slate-800 p-6 rounded-3xl space-y-4 shadow-xl">
            ...
            <div className="space-y-2">
              {savedCards.map((card, idx) => (
                <div key={idx} className="bg-slate-950 p-4 rounded-xl text-sm text-slate-300 border border-slate-800">
                  {card}
                </div>
              ))}
            </div>
