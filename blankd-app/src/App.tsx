...
class ErrorBoundary extends React.Component<{children: React.ReactNode}, {hasError: boolean, error: Error | null}> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="p-10 text-rose-500 bg-black min-h-screen font-mono">
          <h1 className="text-2xl font-bold mb-4">🚨 앱 렌더링 치명적 에러 발생</h1>
          <p className="mb-4 text-white/80">아래 에러 메시지를 긁어서 아키에게 알려주세요.</p>
          <pre className="bg-rose-950/30 p-4 border border-rose-500/50 rounded overflow-auto whitespace-pre-wrap text-[11px]">
            {this.state.error?.toString()}
            {"\n\n"}
            {this.state.error?.stack}
          </pre>
          <button onClick={() => window.location.reload()} className="mt-6 px-6 py-2 bg-rose-600 text-white rounded">새로고침</button>
        </div>
      );
    }
...
function MainApp() {
  ...
  const getColSpanAndStartClass = (title: string, currentViewMode: string, isExpanded: boolean, colCount: number) => {
    if (isExpanded) {
      if (colCount === 2) return "col-span-1 sm:col-span-2";
      if (colCount >= 3) return "col-span-1 sm:col-span-2 md:col-span-3";
      return "col-span-1";
    }
    if (currentViewMode === 'all' && colCount === 3 && title) {
      if (title.includes('[법]')) return "md:col-start-1 col-span-1";
      if (title.includes('[령]')) return "md:col-start-2 col-span-1";
      if (title.includes('[칙]')) return "md:col-start-3 col-span-1";
    }
    return "col-span-1";
  }; 
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0f] text-[#d1d1d1] font-sans selection:bg-neutral-800 selection:text-white p-6 sm:p-12 relative">
            ...
            {activeTab === 'dashboard' && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 animate-in fade-in">
                ...
                <div className="border border-amber-900/30 p-8 rounded-sm bg-amber-950/10">
                  ...
                  <div className="text-4xl font-light text-amber-500/80">{safeCards.filter(c => c.level >= 3).length}</div>
                </div>
              </div>
            )} 
                        ...
                        <div className="grid gap-4 mt-4" style={{ gridTemplateColumns: `repeat(${colCount}, minmax(0, 1fr))` }}>
                          {safeCategories
                            ...
                            .map(cat => {
                              ...
                              return (
                                <div key={cat.id} className={`expandable-card relative flex flex-col justify-center ${gridSpanClass} ${isExpanded ? 'items-stretch' : 'items-center'}`}> 
                                  ...
                                  {!isExpanded ? ( 
                                    <button {...createLongPressHandlers(() => handleDeleteCategory(cat.id), 800)} onClick={() => { setExpandedCategoryId(cat.id); setSelectedWordIndices(new Set()); setParsedText(cat.content || ""); }} className="w-full h-full text-[13px] font-serif font-bold text-center text-indigo-300 bg-indigo-900/20 py-4 px-3 rounded-sm border border-indigo-500/30 shadow-[0_0_10px_rgba(99,102,241,0.1)] transition-all hover:bg-indigo-900/40"> 
                          ...
                          .map((card) => { 
                            ...
                            return ( 
                              <div key={card.id} className="relative expandable-card"> 
                                ...
                                <button {...createLongPressHandlers(() => handleDeleteCard(card.id), 800)} onClick={() => setActiveCard(card)} className={`w-full relative text-[13px] font-serif font-bold text-center py-5 px-3 rounded-sm border shadow-[0_0_10px_rgba(0,0,0,0.5)] transition-all select-none ${card.status === "BURNED" ? "border-white/5 text-white/30" : "border-indigo-500/30 text-indigo-300 bg-indigo-900/20 hover:bg-indigo-900/40"}`}>
      ...
      <style>{`@keyframes shake { 0%, 100% { transform: translateX(0); } 25% { transform: translateX(-5px); } 50% { transform: translateX(5px); } 75% { transform: translateX(-5px); } } .shake-animation { animation: shake 0.3s ease-in-out; } .scrollbar-hide::-webkit-scrollbar { display: none; } .scrollbar-hide { -ms-overflow-style: none; scrollbar-width: none; }`}</style>
    </div>
  );
} 
