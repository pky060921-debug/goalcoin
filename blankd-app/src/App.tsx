...
export default function App() {
  ...
  const handleMakeBlankCard = async (
    ...
    // 💡 덮어쓰기 방식을 위해 동일한 조항에서 만들어진 기존 카드(ORIG_ID가 같은 카드)가 있는지 확인
          ...
          renderContent={() => {
            ...
            parts.forEach((part, i) => {
              ...
              if (part.startsWith('[') && part.endsWith(']')) {
                blankCount++;
                const isCurrent = blankCount === currentBlankIndex;
                const isPassed = blankCount < currentBlankIndex;
                const rawCorrectAnswer = part.slice(1, -1);
                
                if (isPassed && statsRef.current.wrongIndices.has(blankCount)) {
                   currentContent.push(<span key={i} className="inline-block mx-1 font-bold text-red-400 border-b-2 border-red-500/50 line-through decoration-red-500/80">{rawCorrectAnswer}</span>);
                } else if (isPassed) {
                   currentContent.push(<span key={i} className="inline-block mx-1 font-bold text-teal-400 border-b border-teal-500/30">{rawCorrectAnswer}</span>);
                } else if (isCurrent) {
                  currentContent.push(
                    <span key={i} className={`inline-block mx-1 min-w-[3rem] text-center border-b-2 font-bold transition-all px-2 bg-black/40 ${inputStatus === 'correct' ? 'border-teal-500 text-teal-400 scale-110 shadow-[0_0_10px_rgba(20,184,166,0.5)]' : inputStatus === 'wrong' ? 'border-red-500 text-red-400 scale-95 shadow-[0_0_10px_rgba(239,68,68,0.5)]' : 'border-amber-500 text-amber-300 animate-pulse'}`}>
                      {inputStatus === 'idle' ? '?' : inputStatus === 'correct' ? 'O' : 'X'}
                    </span>
                  );
                } else {
                  currentContent.push(<span key={i} className="inline-block mx-1 min-w-[2.5rem] bg-white/5 border border-white/10 rounded px-1 text-transparent select-none">___</span>);
                }

                if (isCurrent && pageCount !== displayPage) {
                   setDisplayPage(pageCount);
                }

              } else {
                currentContent.push(<span key={i} className="text-white/80">{part}</span>);
              }
            });
            
            if (currentContent.length > 0) pages.push(currentContent);
            ...
            return (
              <div className="flex flex-col gap-6 w-full">
                ...
                <div className="font-serif text-[14px] sm:text-[16px] leading-[2.2] break-keep bg-[#050505] p-5 sm:p-8 rounded-sm border border-white/5 shadow-inner min-h-[30vh]">
                  {pages[displayPage]}
                </div>

                <div className="flex flex-wrap gap-2 items-center w-full">
                  <form onSubmit={(e) => { e.preventDefault(); handleSequentialInput(); }} className="flex-1 flex gap-2 relative">
                    <input 
                      id="answer-input"
                      type="text" 
                      value={answerInput} 
                      onChange={e => setAnswerInput(e.target.value)} 
                      className={`flex-1 bg-black/60 border p-3 sm:p-4 text-sm sm:text-base text-amber-200 outline-none rounded shadow-inner transition-colors ${inputStatus === 'wrong' ? 'border-red-500/50 bg-red-950/20' : 'border-amber-500/40 focus:border-amber-400'}`}
                      placeholder={isListening ? "음성 인식 중... (또는 타이핑)" : "정답 입력 후 Enter..."} 
                      autoFocus 
                      autoComplete="off"
                    />
                    <button type="submit" className="px-5 sm:px-8 bg-amber-600/20 text-amber-400 border border-amber-500/30 font-bold rounded hover:bg-amber-600/40 transition-all">
                      입력
                    </button>
                  </form>
                  ...
                  <button 
                    ...
                    className={`px-3 py-1.5 sm:px-4 sm:py-2 border rounded-sm text-[11px] sm:text-xs font-bold transition-all shadow-md shrink-0 flex items-center justify-center gap-1 ${isListening ? 'bg-red-900/40 border-red-500/50 text-red-300 animate-pulse' : 'bg-indigo-900/30 border-indigo-500/40 text-indigo-300 hover:bg-indigo-900/50'}`}
                    title="음성으로 빈칸을 채웁니다."
                  >
                ...
                <div className="flex justify-end pt-2 w-full">
                  <button onClick={() => setIsMemoOpen(!isMemoOpen)} className="text-[10px] sm:text-[11px] text-white/40 hover:text-white/80 transition-colors border border-white/10 px-2 py-1 rounded">
                    {isMemoOpen ? '메모 닫기' : '📝 오답 노트 / 메모'}
                  </button>
                </div>
              </div>
            );
