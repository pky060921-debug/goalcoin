...
// 💡 [제목 추출] 제X조(목적)* 기호까지 본문 섞임 없이 완벽하게 발라냅니다.
...
// 💡 [폴더 정렬] 제1장, 제2장, 제10장을 문자순이 아닌 숫자순으로 완벽하게 나열
...
// 💡 [핵심 패치 1] 같은 줄에 위치할 수 있도록 조항 번호를 숫자로 완벽 환산 (아키님이 인정하신 로직)
...
// 💡 [핵심 패치 2] 인라인 스타일을 사용하여 Tailwind의 삭제(Purge) 버그를 원천 차단
export const getGridStyle = (text: string, studyMode: string, isExpanded: boolean) => {
  ...
  // 절대 지정석 부여
  if (isLaw) return { gridColumnStart: 1, gridColumnEnd: "span 1" };
  if (isDecret) return { gridColumnStart: 2, gridColumnEnd: "span 1" };
  if (isRule) return { gridColumnStart: 3, gridColumnEnd: "span 1" };
  return {};
};
