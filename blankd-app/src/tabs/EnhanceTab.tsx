...
export const EnhanceTab = ({ savedCards, colCount, viewMode, setActiveCard, setActiveTab, setExpandedId, loadAllData, safeAddress, globalDict }: any) => {
  ...
  const handleFindAndReplace = async (e: any) => {
    e.preventDefault();
    const findText = window.prompt("찾 단어(또는 기호)를 입력하세요:");
    if (!findText) return;
  ...
  const handleAddAdjacent = (folderCards: any[], index: number, folder: string) => {
    ...
    const newCard = {
        id: tempId, folder_name: folder,
        content: `${newTitle}\n\n내용을 입력하세요.`, 
        memo: JSON.stringify({ text: "", filled: 0, wrongIndices: [] }),
        answer_text: "", isTemp: true, insertAfterId: origCard.id
    };
  ...
  const handleSaveEdit = async (card: any) => {
    ...
    try {
      ...
      const payload = { 
        wallet_address: safeAddress || "ENOKI_USER", card_id: isTemp ? null : parseInt(card.id, 10), 
        card_content: sanitizedContent, answer_text: newAnswers, folder_name: card.folder_name, memo: card.memo 
      };
      ...
      setEditingId(null); setShowJeonggwanSelector(false);
  ...
  const renderInteractiveText = () => {
    ...
    return (
      <div className={`w-full bg-black/40 p-4 rounded border border-white/10 leading-loose font-sans ${activeTool ? 'select-none' : ''} min-h-[160px] max-h-[400px] overflow-y-auto custom-scrollbar`}>
        <div className="text-amber-400 font-bold mb-2 pb-2 border-b border-white/10 select-none opacity-70 cursor-not-allowed">{titleLine}</div>
            ...
            <span key={idx} onClick={(e) => {
                ...
                if (e.shiftKey) {
                  ...
                  if (currentText.includes(' ')) {
                     const parts = currentText.split(/(\s+)/); let replacement = "";
                     parts.forEach(p => { if (/^\s+$/.test(p)) replacement += p; else replacement += isBracketed ? `[${p}]` : p; });
                     const newTokens = [...tokens]; newTokens[idx] = replacement;
              ...
              onContextMenu={(e) => {
                if (activeTool !== 'smart') return;
                e.preventDefault();
                let nextIdx = idx + 1; let spaces = "";
                while (nextIdx < tokens.length) {
                   if (tokens[nextIdx] === '---' || tokens[nextIdx] === '\n') return; 
                   if (/^\s+$/.test(tokens[nextIdx])) { spaces += tokens[nextIdx]; nextIdx++; } else break; 
                }
                if (nextIdx < tokens.length) {
                   ...
                   for (let i = idx + 1; i <= nextIdx; i++) newTokens[i] = ""; 
                   setEditContent(lines[0] + '\n' + newTokens.join(''));
                }
              }}
              className={btnClass}
          ...
          <div className={`grid grid-cols-1 ${getGridClass(colCount)} gap-1.5 sm:gap-2 items-start w-full`}>
            {localCards.filter((c:any) => c && c.content && c.folder_name === folder).map((card: any, idx: number, folderCards: any[]) => {
                try {
                  ...
                  if (firstLine.includes('[정관]')) { colClass = "md:col-start-1 md:col-span-1"; titleColor = "text-yellow-500"; } 
                  else if (firstLine.includes('[칙]') || firstLine.includes('[규]') || firstLine.includes('[규정]')) { colClass = "md:col-start-3 md:col-span-1"; titleColor = "text-green-500"; } 
                  else if (firstLine.includes('[령]')) { colClass = "md:col-start-2 md:col-span-1"; titleColor = "text-blue-400"; } 
                  else { colClass = "md:col-start-1 md:col-span-1"; titleColor = "text-red-500"; }
                        ...
                        <div {...createLongPressHandlers(() => (card.id))} onClick={(e) => { e.stopPropagation(); if (typeof setActiveCard === 'function') setActiveCard(card); }} className={`w-full p-1.5 sm:p-2 rounded-sm border flex flex-col justify-center gap-0.5 cursor-pointer text-left ${movingId === card.id ? "border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.3)] bg-blue-900/30 ring-2 ring-blue-500/50" : hasWrong ? "border-red-500/40 bg-red-900/20" : "border-indigo-500/30 bg-indigo-900/20 hover:bg-indigo-900/40"} shadow-sm transition-all duration-200`}>
                          ...
                          {movingId === card.id ? (
                            <div className="flex items-center justify-between w-full pt-1 animate-in fade-in">
                              <span className="text-blue-300 text-[10px] font-bold flex items-center">방향키(↑, ↓)로 이동 후 Enter 입력</span>
                              <button onClick={(e) => { e.stopPropagation(); setMovingId(null); if(loadAllData) loadAllData(); }} className="px-2 py-0.5 bg-blue-500 text-white text-[9px] font-bold rounded-sm shadow-md hover:bg-blue-400 transition-colors">완료</button>
                            </div>
                          ) : (
