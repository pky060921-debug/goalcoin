                      ...
                      return (
                        <div key={card.id} id={`enhance-card-${card.id}`} className={`relative transition-all w-full ${colClass}`}>
                          {editingId === card.id ? (
                            ...
                            // 💡 [레이아웃 1줄 압축] flex-row 로 변경하고 통계창을 오른쪽으로 밀어넣습니다.
                            <div {...createLongPressHandlers(() => (card.id))} onClick={(e) => { e.stopPropagation(); if (typeof setActiveCard === 'function') setActiveCard(card); }} className={`w-full p-2 sm:p-2.5 rounded-sm border flex flex-col justify-center cursor-pointer text-left ${movingId === card.id ? "border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.3)] bg-blue-900/30 ring-2 ring-blue-500/50" : hasWrong ? "border-red-500/40 bg-red-900/20" : "border-indigo-500/30 bg-indigo-900/20 hover:bg-indigo-900/40"} shadow-sm transition-all duration-200`}>
                              
                              <div className="flex flex-row items-center justify-between w-full gap-2 overflow-hidden">
                                <div className={`${titleColor} font-bold ${titleSizing} text-left truncate leading-tight flex-1`} title={displayTitle}>
                                  {displayTitle}
                                </div>
                                
                                {movingId === card.id ? (
                                  <div className="flex items-center gap-2 shrink-0 animate-in fade-in">
                                    <span className="text-blue-300 text-[10px] font-bold hidden sm:flex items-center">
                                      방향키(↑, ↓) 이동 후 Enter
                                    </span>
