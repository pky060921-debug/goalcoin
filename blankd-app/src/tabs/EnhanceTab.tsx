...
export const EnhanceTab = ({ savedCards, colCount, viewMode, setActiveCard, setActiveTab, setExpandedId, loadAllData, safeAddress, globalDict }: any) => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-in fade-in w-full h-auto">
          ...
          {(() => {
            ...
            const renderCardNode = (card: any, isFullWidth: boolean = false) => {
              ...
              try {
                ...
                return (
                  <div key={card.id} id={`enhance-card-${card.id}`} className="relative transition-all w-full mb-2 h-auto">
                    {editingId === card.id ? (
                      <div className="relative flex flex-col p-4 rounded-sm border border-amber-500/50 bg-[#0a0a0c] transition-all duration-300 w-full shadow-[0_0_15px_rgba(245,158,11,0.15)] h-auto">
                        ...
                        {activeTool === 'editor' ? (
                          <textarea value={editContent} onChange={(e) => { setEditContent(e.target.value); }} className="w-full min-h-[160px] max-h-[400px] bg-black/60 text-amber-50 text-[12px] p-4 rounded border border-white/10 outline-none resize-none custom-scrollbar h-auto" placeholder="직접 입력하거나 [ ] 기호로 감싸세요. (아예 띄어쓰기가 없는 문장에 빈 공간을 넣을 땐 여기서 입력하세요.)" />
                      ...
                      <button {...createLongPressHandlers(() => (card.id))} onClick={(e) => { e.stopPropagation(); if (typeof setActiveCard === 'function') setActiveCard(card); }} className={`w-full p-1.5 sm:p-2 rounded-sm border flex flex-col justify-center gap-0.5 h-auto ${movingId === card.id ? "border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.3)] bg-blue-900/30 ring-2 ring-blue-500/50" : hasWrong ? "border-red-500/40 bg-red-900/20" : "border-indigo-500/30 bg-indigo-900/20 hover:bg-indigo-900/40"} shadow-sm transition-all duration-200`}>
                        <div className="flex w-full overflow-hidden mb-1 h-auto">
                          <div className={`${titleColor} font-bold ${titleSizing} w-full text-left truncate leading-tight h-auto`} title={displayTitle}>
                            {displayTitle}
                          </div>
                        </div>
                        
                        {movingId === card.id ? (
                          <div className="flex items-center justify-between w-full pt-1 animate-in fade-in h-auto">
                          ...
                          <div className="flex flex-col w-full pt-1 border-t border-white/5 h-auto">
                ...
                return <div key={card.id || Math.random()} className="text-red-500 text-xs p-2 border border-red-500/50 bg-red-900/20 h-auto">카드 렌더링 오류 진단: {renderError.message}</div>;
              }
            };
            ...
            return (
              <div className="flex flex-col gap-4 h-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-4 items-start w-full h-auto">
                  
                  {/* 1열: 법 / 정관 */}
                  <div className="flex flex-col w-full bg-white/5 rounded-sm p-1.5 border border-white/5 h-auto">
                    <div className="text-[10px] text-white/30 font-bold mb-2 text-center tracking-widest border-b border-white/5 pb-1 h-auto">법 / 정관</div>
                    <div className="flex flex-col gap-2 h-auto">
                      {col1Cards.filter(c => c.id !== editingId).map(c => renderCardNode(c))}
                      {col1Cards.length === 0 && <div className="text-center text-white/20 text-[10px] py-4 h-auto">카드가 없습니다.</div>}
                    </div>
                  </div>
                  
                  {/* 2열: 시행령 */}
                  <div className="flex flex-col w-full bg-white/5 rounded-sm p-1.5 border border-white/5 h-auto">
                    <div className="text-[10px] text-white/30 font-bold mb-2 text-center tracking-widest border-b border-white/5 pb-1 h-auto">시행령</div>
                    <div className="flex flex-col gap-2 h-auto">
                      {col2Cards.filter(c => c.id !== editingId).map(c => renderCardNode(c))}
                      {col2Cards.length === 0 && <div className="text-center text-white/20 text-[10px] py-4 h-auto">카드가 없습니다.</div>}
                    </div>
                  </div>
                  
                  {/* 3열: 시행규칙 / 규정 */}
                  <div className="flex flex-col w-full bg-white/5 rounded-sm p-1.5 border border-white/5 h-auto">
                    <div className="text-[10px] text-white/30 font-bold mb-2 text-center tracking-widest border-b border-white/5 pb-1 h-auto">시행규칙 / 규정</div>
                    <div className="flex flex-col gap-2 h-auto">
                      {col3Cards.filter(c => c.id !== editingId).map(c => renderCardNode(c))}
                      {col3Cards.length === 0 && <div className="text-center text-white/20 text-[10px] py-4 h-auto">카드가 없습니다.</div>}
                    </div>
                  </div>
