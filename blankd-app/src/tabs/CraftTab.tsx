...
// 💡 [오류 진단 및 제목 추출 함수] 기존 로직을 파괴하지 않고 예쁜 제목만 안전하게 추출합니다.
const renderCraftTitle = (catObj: any) => {
    try {
        let t = catObj.title || catObj.category_name || "제목 없음";
        return t.replace(/\[.*?\]/g, '').trim(); // '[법]', '[령]' 등 태그만 제거
    } catch(err) {
        console.error("[CraftTab] 조항명 진단 에러:", err, catObj);
        return "제목 렌더링 오류";
    }
};
    ...
    for (const catId of selectedCatIds) {
      ...
      try {
        ...
        if (selectedWordsForCat.size > 0) {
           ...
           await handleMakeBlankCard(cat, wordArray.map(w => w.text), selectedWordsForCat, new Set(), "");
           successCount++;
        }
        
      } catch (e: any) {
         addLog(`⚠️ [${renderCraftTitle(cat)}] 생성 실패: ${e.message}`);
      }
    }
